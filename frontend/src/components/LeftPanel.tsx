    ...
    <div style={{
      ...
      <style>{`
        ...
        input[type=range].split-slider-track::-moz-range-thumb {
          ...
          background: #e9e9e9 !important; cursor: ew-resize;
        }
      `}</style>
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.10em', textTransform: 'uppercase', color: '#555', margin: '0 0 8px' }}>{children}</p>
  );
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div
        ...
        style={{
          ...
          height: 56, background: ROW_BG, cursor: 'pointer',
      ...
      >
        ...
        <UploadIcon />
        <span style={{ fontSize: 11, color: loading ? ACCENT : FG_DIM, fontWeight: 500 }}>
  ...
  return (
    <div
      ...
      style={{
        ...
        background: selected ? '#1e100e' : hovered ? '#141414' : ROW_BG,
        border: `1px solid ${selected ? ACCENT + '55' : ROW_BORDER}`,
        transition: 'background 0.1s, border-color 0.1s',
      }}
    >
      {/* Thumbnail */}
      <div style={{ width: 36, height: 36, borderRadius: 7, overflow: 'hidden', flexShrink: 0, background: '#0a0a0a' }}>
        {item.isVideo
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: selected ? ACCENT : '#bbb', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {item.name || (item.isVideo ? 'Video' : 'Image')}
        </div>
        <div style={{ fontSize: 9, fontFamily: 'monospace', color: '#444', marginTop: 2 }}>
          {item.srcW}×{item.srcH}
        </div>
      </div>
      ...
      <button
        ...
        style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 3, color: '#444', display: 'flex', borderRadius: 5, flexShrink: 0 }}
        onMouseEnter={(e) => (e.currentTarget.style.color = '#e94f37')}
      ...
      <div style={{ display: 'flex', gap: 3, background: ROW_BG, borderRadius: 12, padding: 4, border: `1px solid ${ROW_BORDER}` }}>
        {BG_TYPES.map((t) => (
          <button key={t} onClick={() => onBackground({ ...background, type: t })}
            style={{ flex: 1, padding: '5px 0', borderRadius: 8, fontSize: 10, fontWeight: 600, border: 'none', cursor: 'pointer', letterSpacing: '0.02em', background: type === t ? ACCENT : 'transparent', color: type === t ? '#fff' : '#666', transition: 'background 0.15s, color 0.15s', textTransform: 'capitalize' }}
          >{t}</button>
        ))}
      </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, padding: '10px 12px', background: ROW_BG, borderRadius: 12, border: `1px solid ${ROW_BORDER}` }}>
          {SOLID_PRESETS.map((c) => (
            <button key={c} onClick={() => onBackground({ type: 'solid', color: c })}
              style={{ width: 26, height: 26, borderRadius: 8, background: c, border: 'none', cursor: 'pointer', boxShadow: background.color === c ? `0 0 0 2px ${ACCENT}` : '0 0 0 1px rgba(255,255,255,0.08)', transition: 'box-shadow 0.12s' }} />
          ))}
          <label style={{ position: 'relative', width: 26, height: 26, borderRadius: 8, cursor: 'pointer', overflow: 'hidden', flexShrink: 0, background: type === 'solid' ? (background.color || '#1a1a2e') : '#2a2a2a', boxShadow: '0 0 0 1px rgba(255,255,255,0.08)' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, padding: '10px 12px', background: ROW_BG, borderRadius: 12, border: `1px solid ${ROW_BORDER}` }}>
            {GRAD_PRESETS.map((g, i) => (
              <button key={i} onClick={() => onBackground({ type: 'gradient', gradient: g })}
                style={{ width: 40, height: 26, borderRadius: 8, border: 'none', cursor: 'pointer', background: `linear-gradient(${g.direction}deg,${g.from},${g.to})`, boxShadow: (background.gradient?.from === g.from && background.gradient?.to === g.to) ? `0 0 0 2px ${ACCENT}` : '0 0 0 1px rgba(255,255,255,0.08)', transition: 'box-shadow 0.12s' }} />
            ))}
          </div>
          <Row label="Colors">
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 4 }}>
        {BASE_RATIOS.map(({ value, label, w, h }) => (
          <button key={value} onClick={() => onCanvas({ ratio: value, width: w, height: h })} style={chipBtnStyle(canvas.ratio === value)}>{label}</button>
        ))}
      </div>

      <button onClick={() => setExpanded((v) => !v)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 12px', background: (expanded || isSocial) ? '#1e1010' : ROW_BG, borderRadius: 11, border: `1px solid ${(expanded || isSocial) ? ACCENT + '55' : ROW_BORDER}`, cursor: 'pointer', transition: 'background 0.15s, border-color 0.15s' }}>
        <span style={{ fontSize: 12, color: (expanded || isSocial) ? ACCENT : '#888', fontWeight: 600 }}>Social media</span>
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{ transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', color: '#555' }}>
        ...
        {expanded && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 4 }}>
            {SOCIAL_GROUPS.map(({ id, name, icon, presets }) => (
              <div key={id} style={{ padding: '8px 10px', background: ROW_BG, borderRadius: 11, border: `1px solid ${ROW_BORDER}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>{icon}<span style={{ fontSize: 11, fontWeight: 600, color: '#999' }}>{name}</span></div>
                  ...
                  {presets.map(({ value, label, w, h }) => {
                    ...
                    return (
                      <button key={value} onClick={() => onCanvas({ ratio: value, width: w, height: h })}
                        style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', padding: '7px 9px', borderRadius: 8, cursor: 'pointer', background: isActive ? ACCENT : '#1a1a1a', border: isActive ? 'none' : '1px solid rgba(255,255,255,0.05)', transition: 'background 0.12s' }}>
...
function ExportButton({ hasItems, onExport }: { hasItems: boolean; onExport: () => void }) {
  ...
  return (
    <button onClick={onExport} disabled={!hasItems} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ width: '100%', padding: '12px 0', borderRadius: 14, fontSize: 13, fontWeight: 700, border: 'none', letterSpacing: '0.05em', textTransform: 'uppercase', background: !hasItems ? '#161616' : ACCENT, color: !hasItems ? '#333' : '#fff', cursor: !hasItems ? 'not-allowed' : 'pointer', transition: 'box-shadow 0.18s, opacity 0.12s', boxShadow: (hasItems && hovered) ? `0 0 22px 2px ${ACCENT}44` : 'none', opacity: (hasItems && hovered) ? 0.9 : 1 }}>
      Export
    </button>
  );
}
...
function BgImageDrop({ currentFileId, onUploaded }: { currentFileId?: string; onUploaded: (id: string) => void }) {
  ...
  return (
    <div {...getRootProps()} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 12, border: `1.5px dashed ${isDragActive ? ACCENT : 'rgba(255,255,255,0.1)'}`, background: isDragActive ? '#1a0f0d' : ROW_BG, cursor: 'pointer', transition: 'all 0.15s' }}>
      <input {...getInputProps()} />
      {currentFileId
        ? <img src={`/api/download/${currentFileId}`} style={{ width: 36, height: 36, borderRadius: 8, objectFit: 'cover', flexShrink: 0 }} />
        : <div style={{ width: 36, height: 36, borderRadius: 8, background: '#1e1e1e', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><UploadIcon /></div>
      }
      <div>
        <p style={{ fontSize: 11, fontWeight: 600, color: FG, margin: 0 }}>{isDragActive ? 'Drop to set' : currentFileId ? 'Background set' : 'Drop image'}</p>
...
function chipBtnStyle(active: boolean): React.CSSProperties {
  return { padding: '7px 4px', borderRadius: 10, fontSize: 10, fontWeight: 600, border: active ? 'none' : `1px solid ${ROW_BORDER}`, cursor: 'pointer', letterSpacing: '0.02em', background: active ? ACCENT : ROW_BG, color: active ? '#fff' : '#888', transition: 'background 0.12s, color 0.12s' };
}
